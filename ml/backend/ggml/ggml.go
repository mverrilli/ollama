...
func (t *Tensor) TQDequantOutlier(ctx ml.Context, scales, codebook ml.Tensor, headDim, numKVHeads, nCells, firstCell, bits int,
	...
	zeros, outlierZeros ml.Tensor,
	qjlPacked, qjlNorm, qjlProjection ml.Tensor, qjlRows int) ml.Tensor {
...
// Q8KEncode creates a GGML_OP_Q8K_ENCODE graph node.
// t = packed buffer [headDim * numKVHeads, capacity] i8 (dst, view returned)
// scales = [(headDim/32) * numKVHeads, capacity] f16 (side-output)
// mins   = [(headDim/32) * numKVHeads, capacity] f16 (side-output)
// k      = [headDim, numKVHeads, batchSize] f16
func (t *Tensor) I4Encode(ctx ml.Context, scales, mins, k ml.Tensor, firstCell int) ml.Tensor {
	return &Tensor{
		b: t.b,
		t: C.ggml_q8k_encode(
			ctx.(*Context).ctx,
...
// Q8KDequant creates a GGML_OP_Q8K_DEQUANT graph node.
// t = encode result (view of packed, establishes graph dependency)
// scales, mins = tensors from Q8KEncode
// Returns [headDim, numKVHeads, nCells] f16.
func (t *Tensor) I4Dequant(ctx ml.Context, scales, mins ml.Tensor, headDim, numKVHeads, nCells, firstCell int) ml.Tensor {
	return &Tensor{
		b: t.b,
		t: C.ggml_q8k_dequant(
			ctx.(*Context).ctx,
			t.t,
			scales.(*Tensor).t,
			mins.(*Tensor).t,
...
// Q4KDequant creates a GGML_OP_Q4K_DEQUANT graph node.
// t = encode result (view of packed, establishes graph dependency)
// scales, mins = tensors from Q4KEncode
// Returns [headDim, numKVHeads, nCells] f16.
func (t *Tensor) I4Q4Dequant(ctx ml.Context, scales, mins ml.Tensor, headDim, numKVHeads, nCells, firstCell int) ml.Tensor {
	return &Tensor{
		b: t.b,
		t: C.ggml_q4k_dequant(
			ctx.(*Context).ctx,
			t.t,
			scales.(*Tensor).t,
			mins.(*Tensor).t,
...
// Q4KEncode creates a GGML_OP_Q4K_ENCODE graph node.
// t = packed buffer [(headDim/2) * numKVHeads, capacity] u8 (nibble-packed, dst view returned)
// scales = [(headDim/32) * numKVHeads, capacity] f16 (side-output)
// mins   = [(headDim/32) * numKVHeads, capacity] f16 (side-output)
// k      = [headDim, numKVHeads, batchSize] f16 or f32
func (t *Tensor) I4Q4Encode(ctx ml.Context, scales, mins, k ml.Tensor, firstCell int) ml.Tensor {
	return &Tensor{
		b: t.b,
		t: C.ggml_q4k_encode(
			ctx.(*Context).ctx,
