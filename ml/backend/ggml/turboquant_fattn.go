...
type tqTensor struct {
	*Tensor           // packed K view ([packedBytes*nKVHeads, capacity] i8; encode result)
	...
	// channelScale is the per-channel W_K column norm [headDim] f32 for G's
	// pre-WHT normalization approach. K is divided by channelScale before WHT;
	// Q is multiplied by channelScale before Q's WHT so Q·K^T is preserved.
	// nil = no per-channel scaling.
	channelScale *Tensor
