...
type ggmlTQCompressedK struct {
	...
	// kChannelScaleTensors is retained as an always-empty map: the per-channel
	// K-scale sources (W_K norms, SCALES_FILE, auto-calib) were removed as a
	// closed experiment that regressed quality. Reads return nil (no scaling).
	// TODO: remove together with the k_channel_scale kernel/op param.
	kChannelScaleTensors map[int]*Tensor
...
func (b *Backend) NewTQCompressedKManager(headDim, numKVHeads, bits int, rotationSeed uint64, vBits, outlierBits, outlierCount int, asymmetricPrimary bool) ml.TQCompressedKManager {
	...
	m := &ggmlTQCompressedK{
		...
		kChannelScaleTensors:    make(map[int]*Tensor),
...
func (m *ggmlTQCompressedK) encodeKInternal(ctx ml.Context, layer int, key ml.Tensor, firstCell int, locs ml.Tensor) ml.Tensor {
	...
	if m.hasOutliers() {
		if oPacked := m.outlierPackedTensors[layer]; oPacked != nil {
			...
			var kCScale ml.Tensor
			if t := m.kChannelScaleTensors[layer]; t != nil {
				kCScale = t
			}
	...
	return packed.TQEncode(ctx, scales, key, m.rotFor(layer), firstCell, m.boundariesTensor, m.bits, zeros, kBias, m.codebookTensor, locs, kCScale)
}
	...
	if m.hasOutliers() {
		if oPacked := m.outlierPackedTensors[layer]; oPacked != nil {
			...
			k := encodeResult.(*Tensor).TQDequantOutlier(ctx, pScales, m.codebookTensor,
				...
				zeros, outlierZeros,
				whtSigns, locs)
			if cs := m.kChannelScaleTensors[layer]; cs != nil && k != nil {
				k = k.Mul(ctx, cs)
			}
			return k
		}
...
func (m *ggmlTQCompressedK) getAsTQTensorInternal(ctx ml.Context, layer int, encodeResult ml.Tensor, firstCell, nCells int, locs ml.Tensor) (ml.Tensor, bool) {
	...
	t := &tqTensor{
		...
		channelScale: m.kChannelScaleTensors[layer],
	}
	if locs != nil {
		t.locs = locs.(*Tensor)
	}
...
func (m *ggmlTQCompressedK) encodeKVInternal(ctx ml.Context, layer int, key, value ml.Tensor, firstCell int, locs ml.Tensor) (ml.Tensor, ml.Tensor) {
	...
	kResult := kPacked.TQEncodeKV(ctx,
		...
		firstCell, m.bits, m.vBits, kBias, m.codebookTensor, m.vCodebookTensor, locs, kCScale)
...
func (m *ggmlTQCompressedK) dequantKVInternal(ctx ml.Context, layer int, kEncodeResult, vEncodeResult ml.Tensor, firstCell, nCells int, locs ml.Tensor) (ml.Tensor, ml.Tensor) {
	...
	if cs := m.kChannelScaleTensors[layer]; cs != nil {
		kView = kView.Mul(ctx, cs)
	}
