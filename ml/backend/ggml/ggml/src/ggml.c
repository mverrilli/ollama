...
struct ggml_tensor * ggml_tq_encode(
        ...
        struct ggml_tensor  * k_channel_scale) {
    ...
    result->src[7] = k_channel_scale; // NULL = no per-channel scaling; [headDim] f32 W_K column norms
    // src[8..9] reserved.
...
struct ggml_tensor * ggml_tq_encode_outlier(
    ...
    result->src[10] = locs;           // NULL = contiguous (cell = firstCell+i); [batchSize] i32
    result->src[11] = k_channel_scale; // NULL = no per-channel scaling; [headDim] f32 W_K column norms
    // src[12] is reserved.
...
struct ggml_tensor * ggml_tq_encode_kv(
    ...
    result->src[12] = k_channel_scale; // NULL = no per-channel scaling; [headDim] f32 W_K column norms
