#include "saw4k-dequant.cuh"

// Per-group asymmetric int4 decode for saw4k: nibble → val = nibble * scale + min.
// Dequanted values are in rotated space (FWHT was applied at encode time).
// The fused flash-attention kernel handles the rotation on the Q side.
//
...
//
// Encoded format (must match saw4k-encode.cu):
...
#define SAW4K_DEQUANT_GROUP_SIZE 32

#if __CUDA_ARCH__ >= 600 || !defined(__CUDA_ARCH__)
__global__ void saw4k_dequant_kernel(
    ...
    const int numGroups = headDim / SAW4K_DEQUANT_GROUP_SIZE;
    const int g         = d / SAW4K_DEQUANT_GROUP_SIZE;
...
void ggml_cuda_saw4k_dequant(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    GGML_ASSERT(headDim % SAW4K_DEQUANT_GROUP_SIZE == 0);
    ...
    saw4k_dequant_kernel<<<grid, block, 0, stream>>>(
