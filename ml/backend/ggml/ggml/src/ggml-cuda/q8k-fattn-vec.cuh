...
// K-only fused flash-attention with per-group asymmetric int8 (i4k preset).
//
// Decode: k[d] = packed[base+d] * scale[d/G] + min[d/G]  (G=32)
//
// Storage layout (matches i4-encode.cu):
//   packed  [(cell*numKVHeads + head) * D + d]              u8
...
//
// D=128 only (Qwen2, Llama3 families).

#define Q8K_FA_GROUP_SIZE 32

#if __CUDA_ARCH__ >= 600 || !defined(__CUDA_ARCH__)

template<int D, int ncols, bool use_logit_softcap>
__launch_bounds__(128, 2)
static __global__ void q8k_flash_attn_ext_vec(
    const char     * __restrict__ Q,          // [D, nTokensQ, nHeadsQ, nSeq] f32
    const uint8_t  * __restrict__ K_packed,   // [(cell*nKVH + h)*D + d]  u8
...
)
{
    ...
    constexpr int cpy_nb      = ggml_cuda_get_max_cpy_bytes(); // 16 Volta+, 8 Pascal
    constexpr int cpy_ne      = cpy_nb / 4;
    constexpr int nthreads_KQ = nthreads / cpy_nb;  // 8 Volta+, 16 Pascal
    constexpr int nthreads_V  = nthreads_KQ;
    constexpr int V_rows_per_thread = 2 * cpy_ne;   // 8 Volta+, 4 Pascal
    constexpr int V_cols_per_iter   = WARP_SIZE / nthreads_V;
    constexpr int nwarps      = nthreads / WARP_SIZE; // 4
    constexpr int numGroups   = D / Q8K_FA_GROUP_SIZE; // 4
    ...
    // Scale/min stride per cell: numGroups * nKVHeads.
...
#pragma unroll
    ...
    // Load Q into registers (float2 pairs) for the KQ dot-product loop.
...
#pragma unroll
        for (int i_KQ_0 = 0; i_KQ_0 < nthreads_KQ; ++i_KQ_0) {
            ...
            // Per-cell pointers for K packed bytes and scale/min.
            const uint8_t * packed_row = in_range
                ? K_packed + (int64_t)cell_rel * nKVHeads * D : nullptr;

            // Load per-group scales and mins for this cell.
            // Stride per cell = numGroups * nKVHeads; pointer already at head_kv offset.
            float k_s[numGroups];
            float k_m[numGroups];
            if (in_range) {
...
#pragma unroll
            for (int j = 0; j < ncols; ++j) {
                // Per-query mask: each column j uses its own mask row (ne31 = nCells stride).
                float mask_val = 0.0f;
                if (maskh && in_range) {
...
#pragma unroll
    ...
    // Cross-warp reduce and write output.
