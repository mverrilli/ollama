#include "saw4k-fattn.cuh"
#include "saw4k-fattn-vec.cuh"
#include "ggml-cuda.h"

// Shared memory matches q4k layout (same tile sizes, same accumulator structure).
constexpr size_t SAW4K_FATTN_SMEM_FLOATS = 2560;

template<int D, int ncols>
static void saw4k_fattn_launch(
    ...
    uint64_t signLo, uint64_t signHi)
{
    ...
    if (logit_softcap != 0.0f) {
        saw4k_flash_attn_ext_vec<D, ncols, true><<<blocks, threads,
            SAW4K_FATTN_SMEM_FLOATS * sizeof(float), ctx.stream()>>>(
            (const char *)Q->data,
        ...
        saw4k_flash_attn_ext_vec<D, ncols, false><<<blocks, threads,
            SAW4K_FATTN_SMEM_FLOATS * sizeof(float), ctx.stream()>>>(
            (const char *)Q->data,
            (const uint8_t *)K_p->data,
            (const char *)V->data,
...
void ggml_cuda_saw4k_flash_attn_ext(ggml_backend_cuda_context & ctx, ggml_tensor * dst)
{
    GGML_ASSERT(ggml_cuda_info().devices[ctx.device].cc >= 600 &&
                "saw4k fused flash attention requires compute capability 6.0+");
    ...
    if (nTokensQ >= 4) {
        saw4k_fattn_launch<128, 4>(ctx, dst, scale, logit_softcap, firstCell, nCells, nKVHeads, signLo, signHi);
    } else if (nTokensQ >= 2) {
        saw4k_fattn_launch<128, 2>(ctx, dst, scale, logit_softcap, firstCell, nCells, nKVHeads, signLo, signHi);
    } else {
        saw4k_fattn_launch<128, 1>(ctx, dst, scale, logit_softcap, firstCell, nCells, nKVHeads, signLo, signHi);
    }
}
