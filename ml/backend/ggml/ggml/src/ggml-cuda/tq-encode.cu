...
__global__ void tq_encode_kernel(
    ...
    const float     *k_channel_scale, // [headDim] f32 per-channel W_K norm, NULL = no scaling
    ...
    for (int d = threadIdx.x; d < headDim; d += block_size) {
        ...
        if (k_channel_scale != nullptr) {
            float sc = k_channel_scale[d];
            if (sc > 1e-10f) val /= sc;
        }
        s_k[d] = val;
    }
...
void ggml_cuda_tq_encode(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    if (outlierCount > 0 && outlierBits > 0 && outlierCount < headDim) {
        ...
        const struct ggml_tensor * k_channel_scale_t  = dst->src[11]; // per-channel W_K norm (NULL = no scaling)
        // src[12] is reserved.
        ...
        tq_encode_kernel_outlier<<<grid, block_size, smem, stream>>>(
            ...
            locs_ptr,
            k_channel_scale_t ? (const float *)k_channel_scale_t->data : nullptr,
            block_size
        );
        return;
    }
    ...
    // src[7] = per-channel W_K column norm for G's pre-WHT normalization (NULL = no scaling).
    const struct ggml_tensor * k_cscale_t = dst->src[7];
    const float * k_cscale_ptr = k_cscale_t ? (const float *)k_cscale_t->data : nullptr;
...
void ggml_cuda_tq_encode_kv(ggml_backend_cuda_context & ctx, struct ggml_tensor * dst) {
    ...
    const struct ggml_tensor * k_channel_scale_kv  = dst->src[12]; // per-channel W_K norm (NULL = no scaling)
    ...
    const float * k_cscale_kv_ptr   = k_channel_scale_kv ? (const float *)k_channel_scale_kv->data : nullptr;
