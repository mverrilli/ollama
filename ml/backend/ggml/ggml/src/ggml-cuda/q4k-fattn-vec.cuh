...
// K-only fused flash-attention with per-group asymmetric int4 (q4k preset).
//
// Decode: k[d] = nibble(packed[base + d/2], d%2) * scale[d/G] + min[d/G]  (G=32)
// Since d0 = 2*(k*nthreads_KQ + tid_kq) is always even, elements d0 and d0+1
// share one byte: low nibble = d0, high nibble = d0+1.
//
// Storage layout (matches q4k-encode.cu):
...
#define Q4K_FA_GROUP_SIZE 32

#if __CUDA_ARCH__ >= 600 || !defined(__CUDA_ARCH__)

template<int D, int ncols, bool use_logit_softcap>
__launch_bounds__(128, 2)
static __global__ void q4k_flash_attn_ext_vec(
...
)
{
    ...
    constexpr int numGroups   = D / Q4K_FA_GROUP_SIZE;
    ...
    // Packed buffer stride: D/2 bytes per (cell, head).
...
#pragma unroll
        for (int i_KQ_0 = 0; i_KQ_0 < nthreads_KQ; ++i_KQ_0) {
            ...
            // Per-cell pointer: D/2 bytes per (cell, head).
