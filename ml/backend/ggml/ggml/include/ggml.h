...
extern "C" {
    ...
    enum ggml_op {
        ...
        GGML_OP_TQ_WHT,
        // ollama: Phase D — no-op proxy that extends a side-effect tensor's
        // lifetime through a downstream consumer. Used to give TQ encode's
        // side-effect outputs (scales/zeros/outlier_*) graph-edge consumers
        // so gallocr doesn't recycle their memory as scratch before FA reads.
        GGML_OP_TQ_ENCODED_PROXY,
...
GGML_API struct ggml_tensor * ggml_tq_encode(
        ...
        struct ggml_tensor  * k_channel_scale); // NULL = no scaling; [headDim] f32 W_K column norms (G's approach)
...
// Fused TQ-encode + outlier dequant + WHT undo.
// Same as ggml_tq_dequant_outlier but additionally encodes enc_cell in the
// same Metal dispatch, eliminating the TQ_ENCODE→DQ cross-kernel barrier.
// The Metal backend dispatches kernel_tq_enc_dq_wht when src[13] != NULL.
GGML_API struct ggml_tensor * ggml_tq_dequant_outlier_fused_enc(
        struct ggml_context * ctx,
        struct ggml_tensor  * encode_result,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * codebook,
        ...
        struct ggml_tensor  * wht_signs,
        struct ggml_tensor  * locs,
        struct ggml_tensor  * k_new,        // raw K for encode cell (f16 or f32)
        struct ggml_tensor  * boundaries,   // regular quant boundaries
        struct ggml_tensor  * outl_bounds,  // outlier quant boundaries
        int32_t              enc_cell);     // absolute cell index to encode
...
GGML_API struct ggml_tensor * ggml_tq_flash_attn_ext(
        ...
        struct ggml_tensor  * locs,             // NULL = contiguous; [nCells] i32 = indexed slots
        struct ggml_tensor  * wht_signs);       // NULL = no WHT-Q fusion; [headDim] f32 = SHS-WHT signs
...
// a graph dependency on `encode_result`. Used to give TQ encoder's
// side-effect-written tensors (K_scales, K_zeros, outlier_*) graph-edge
// consumers so gallocr extends their memory lifetime through downstream
// readers (Phase D's TQ FA op, Phase B's K-only inline-decode kernel).
// The kernel does nothing at runtime — encode wrote the data; this op just
// establishes the edge.
GGML_API struct ggml_tensor * ggml_tq_encoded_proxy(
        struct ggml_context * ctx,
        struct ggml_tensor  * encode_result,
