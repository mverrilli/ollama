...
extern "C" {
    ...
    enum ggml_op {
        ...
        GGML_OP_Q8K_ENCODE,          // per-group asymmetric int8 encode (no rotation)
        GGML_OP_Q8K_DEQUANT,         // per-group asymmetric int8 decode to f16
        GGML_OP_Q8K_FLASH_ATTN_EXT,  // fused q8k K flash-attention (K inline decode, V f16)

        GGML_OP_Q4K_ENCODE,          // per-group asymmetric int4 encode (nibble packing)
        GGML_OP_Q4K_DEQUANT,         // per-group asymmetric int4 decode to f16
        GGML_OP_Q4K_FLASH_ATTN_EXT,  // fused q4k K flash-attention (K nibble decode, V f16)

        GGML_OP_SAW8K_ENCODE,         // per-group asymmetric int8 encode with FWHT rotation
        GGML_OP_SAW8K_DEQUANT,        // per-group asymmetric int8 decode to f16 (rotated space)
        GGML_OP_SAW8K_FLASH_ATTN_EXT, // fused saw8k K flash-attention (Q rotated inline)

        GGML_OP_SAW4K_ENCODE,         // per-group asymmetric int4 encode with FWHT rotation
        GGML_OP_SAW4K_DEQUANT,        // per-group asymmetric int4 decode to f16 (rotated space)
        GGML_OP_SAW4K_FLASH_ATTN_EXT, // fused saw4k K flash-attention (Q rotated inline)
...
// Per-group asymmetric int8 encode. No rotation, no codebook. Groups of 32 elements.
// packed:  [headDim * numKVHeads, capacity] i8 — one byte per element
// scales:  [(headDim/32) * numKVHeads, capacity] f16 — scale per group per head per cell
// mins:    [(headDim/32) * numKVHeads, capacity] f16 — min per group per head per cell
// k:       [headDim, numKVHeads, batchSize] f16
// firstCell: starting cache slot index
GGML_API struct ggml_tensor * ggml_q8k_encode(
        struct ggml_context * ctx,
        struct ggml_tensor  * packed,
        struct ggml_tensor  * scales,
...
// encode_result: view of packed buffer from ggml_q8k_encode (establishes ordering)
// scales, mins: same tensors passed to ggml_q8k_encode
GGML_API struct ggml_tensor * ggml_q8k_dequant(
        struct ggml_context * ctx,
        struct ggml_tensor  * encode_result,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_q8k_flash_attn_ext(
        struct ggml_context * ctx,
        struct ggml_tensor  * Q,
        struct ggml_tensor  * K_packed,
        struct ggml_tensor  * V,
...
// Per-group asymmetric int4 encode. Groups of 32 elements, 2 nibbles per byte.
// packed:  [headDim/2 * numKVHeads, capacity] i8 — nibble-packed, LSB-first
// scales:  [(headDim/32) * numKVHeads, capacity] f16
// mins:    [(headDim/32) * numKVHeads, capacity] f16
// k:       [headDim, numKVHeads, batchSize] f16
GGML_API struct ggml_tensor * ggml_q4k_encode(
        struct ggml_context * ctx,
        struct ggml_tensor  * packed,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_q4k_dequant(
        struct ggml_context * ctx,
        struct ggml_tensor  * encode_result,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_q4k_flash_attn_ext(
        struct ggml_context * ctx,
        struct ggml_tensor  * Q,
        struct ggml_tensor  * K_packed,
        struct ggml_tensor  * V,
...
// SAW-INT8: per-group asymmetric int8 encode with FWHT rotation preprocessing.
// signLo/signHi: 128 random ±1 sign bits (bit=0 → +1, bit=1 → -1) for elements 0-63 / 64-127.
GGML_API struct ggml_tensor * ggml_saw8k_encode(
        struct ggml_context * ctx,
        struct ggml_tensor  * packed,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_saw8k_dequant(
        struct ggml_context * ctx,
        struct ggml_tensor  * encode_result,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_saw8k_flash_attn_ext(
        struct ggml_context * ctx,
        struct ggml_tensor  * Q,
        struct ggml_tensor  * K_packed,
        struct ggml_tensor  * V,
...
// SAW-INT4: per-group asymmetric int4 encode with FWHT rotation preprocessing.
GGML_API struct ggml_tensor * ggml_saw4k_encode(
        struct ggml_context * ctx,
        struct ggml_tensor  * packed,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_saw4k_dequant(
        struct ggml_context * ctx,
        struct ggml_tensor  * encode_result,
        struct ggml_tensor  * scales,
        struct ggml_tensor  * mins,
...
GGML_API struct ggml_tensor * ggml_saw4k_flash_attn_ext(
        struct ggml_context * ctx,
        struct ggml_tensor  * Q,
        struct ggml_tensor  * K_packed,
        struct ggml_tensor  * V,
