...
// and K projection weights into caches that support TurboQuant-aware encoding
// (e.g. TurboQuantCache). Qwen2 includes a learnable bias on the K projection;
// without bias wiring, TQ's WHT rotation spreads the bias across all channels.
// Weight wiring enables G's per-channel scale (pre-WHT normalization by W_K
// column norms), which reduces per-channel variance before rotation.
